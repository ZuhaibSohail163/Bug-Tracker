  ...
  getTicketComments: async (req, res) => {
    ...
    try {
      const { rows } = await client.query(
        "SELECT comments.id, comments.ticket_id, comments.comment, comments.created_at, users.user_id AS author_id, users.first_name, users.last_name FROM comments JOIN users ON comments.author_id = users.user_id WHERE ticket_id = $1",
        [ticketId]
      );
